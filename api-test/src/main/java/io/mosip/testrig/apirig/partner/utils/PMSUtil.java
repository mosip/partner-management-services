...
public class PMSUtil extends AdminTestUtil {
	...
	public void validateResponse(Response response, String testCaseName, String idKeyName) {
		if (response != null) {
			if (response.asString().contains("PMS_FEATURE_001") || response.asString().contains("PMS_FEATURE_002")
					|| response.asString().contains("PMS_FEATURE_003")) {
				handleIdKeys(testCaseName, idKeyName);
				throw new SkipException(PMSConstants.FEATURE_NOT_SUPPORTED_PMSREVAMP);
			}
			if (response.asString().contains("PMS_FEATURE_004")) {
				handleIdKeys(testCaseName, idKeyName);
				throw new SkipException(PMSConstants.FEATURE_NOT_SUPPORTED_PMSREVAMP_FOR_OIDC);
			}
		}
	}
	...
	private void handleIdKeys(String testCaseName, String idKeyName) {
	    if (idKeyName != null) {
	        ...
	        for (String fieldName : fieldNames) {
	            writeAutoGeneratedId(testCaseName, fieldName.trim(), "randomData");
	        }
	    }
	}
}