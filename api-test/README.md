# Partner Management Services API Test Rig
...
2. Run the automation test suite JAR file:
   ```
   java -jar -Dmodules=pms -Denv.user=api-internal.<env_name> -Denv.endpoint=<base_env> -Denv.testLevel=smokeAndRegression -jar apitest-pms-1.2.2.4-jar-with-dependencies.jar
   ```
...
- **jar**: Specify the name of the JAR file to execute. The version will change according to the development code version. For example, the current version may look like `apitest-pms-1.2.2.4-jar-with-dependencies.jar`.
### Build and Run Info
