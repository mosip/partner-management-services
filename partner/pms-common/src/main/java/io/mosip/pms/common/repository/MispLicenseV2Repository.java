...
public interface MispLicenseV2Repository extends JpaRepository<MISPLicenseEntityV2, String> {
    ...
    @Query(value = "select * from misp_license ml where ml.misp_id=?1 and ml.license_key_name = ?2", nativeQuery = true)
    List<MISPLicenseEntityV2> findByPartnerIdAndLicenseKeyName(String partnerId, String licenseKeyName);
    ...
    @Query(value = "select * from misp_license ml where (ml.is_deleted is null or ml.is_deleted = false) and ml.is_active = true", nativeQuery = true)
    List<MISPLicenseEntityV2> findAllActiveMISPLicenses();
}
