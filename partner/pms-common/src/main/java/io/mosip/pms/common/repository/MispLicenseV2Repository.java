...
public interface MispLicenseV2Repository extends JpaRepository<MISPLicenseEntityV2, String> {
    ...
    @Query(value = "select * from misp_license ml where ml.misp_id=?1 and ((?2 IS NULL AND ml.policy_id IS NULL) OR ml.policy_id = ?2) and ml.license_key_name=?3", nativeQuery = true)
