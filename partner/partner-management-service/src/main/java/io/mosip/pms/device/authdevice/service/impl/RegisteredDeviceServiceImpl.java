...
import io.mosip.pms.device.dto.DeviceResponse;
import io.mosip.pms.device.dto.DigitalId;
import io.mosip.pms.device.dto.JWTSignatureRequestDto;
...
import io.mosip.pms.device.validator.RegisteredDeviceConstant;
...
public class RegisteredDeviceServiceImpl implements RegisteredDeviceService {
	...
	public String signedRegisteredDevice(RegisteredDevicePostDto registeredDevicePostDto) throws Exception {
		...
		try {
			...
			mapEntity = mapRegisteredDeviceDto(registeredDevicePostDto, digitalIdJson, deviceData, deviceInfo,
					deviceDetail, digitalId);
	 ...
	 */
	private String getSignedResponse(String encodedResponse) throws DeviceValidationException {
		List<ServiceError> serviceErrors = new ArrayList<>();
	...
	private RegisteredDevice mapRegisteredDeviceDto(RegisteredDevicePostDto registeredDevicePostDto,
			String digitalIdJson, DeviceData deviceData, DeviceInfo deviceInfo, DeviceDetail deviceDetail,
			DigitalId digitalId) {
	 ...
	 * @throws DeviceValidationException 
	 */
	private void validate(DigitalId digitalId) throws DeviceValidationException {
		List<ServiceError> errors = new ArrayList<>();
		ValidatorFactory factory = Validation.buildDefaultValidatorFactory();
		Validator validator = factory.getValidator();
