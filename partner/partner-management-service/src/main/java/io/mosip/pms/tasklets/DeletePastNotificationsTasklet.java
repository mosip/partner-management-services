...
/**
 * This Batch Job deletes past notifications based on a configured time
 * threshold.
 */
@Component
public class DeletePastNotificationsTasklet implements org.springframework.batch.core.step.tasklet.Tasklet {

	private Logger log = PMSLogger.getLogger(DeletePastNotificationsTasklet.class);
	...
	private void deletePastNotifications() {
		try {
			...
			log.info("Fetching notifications older than {} days (before {}).", pastNotificationDeletionPeriod,
					pastNotificationsDate);

			// Fetch past notifications to delete
			List<NotificationEntity> notificationsToDelete = notificationServiceRepository
					.getPastNotifications(pastNotificationsDate);
			...
			notificationsToDelete.forEach(notification -> {
				try {
					...
					log.error("Failed to delete notification for notification ID {}: {}", notification.getId(),
							e.getMessage());
				}
			});
