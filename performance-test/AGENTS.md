...
3. `auth-demo-service` (from [mosip-functional-tests](https://github.com/mosip/mosip-functional-tests)) must be installed and reachable — the scripts use it to generate auth tokens. Do not pass the client secret as a `-D` process argument — it would be visible in shell history, process listings, and CI logs. Put it in an external properties file instead:
